...
<!-- bds: this theme is broken, and it should be inactive anyway per dspace.cfg -->
...
<xsl:choose>
    ...
    <xsl:when test="$clause = 4">
        <xsl:call-template name="itemFieldDisplay.dc.relation.haspart">
            <xsl:with-param name="clause" select="$clause" />
            <xsl:with-param name="phase" select="$phase" />
            <xsl:with-param name="otherPhase" select="$otherPhase" />
        </xsl:call-template>
    ...
    <xsl:when test="$clause = 5">
        <xsl:call-template name="itemFieldDisplay.dc.source.uri">
            <xsl:with-param name="clause" select="$clause" />
            <xsl:with-param name="phase" select="$phase" />
            <xsl:with-param name="otherPhase" select="$otherPhase" />
        </xsl:call-template>
    ...
    <xsl:when test="$clause = 6">
        <xsl:call-template name="itemFieldDisplay.dc.description.ALL">
            <xsl:with-param name="clause" select="$clause" />
            <xsl:with-param name="phase" select="$phase" />
            <xsl:with-param name="otherPhase" select="$otherPhase" />
        </xsl:call-template>
